...
<%
	...
	inquiryBean ib = idao.getContentByIno(ino); 
...
<td colspan="5" align="center" style="font-family: 'Nanum Pen Script', cursive;font-size: 25px;">
	<b>답변 작성</b>
	<br><br>
<form name="writeform" action="<%= request.getContextPath() %>/MY/Inquiry/inquiry_reply_Proc.jsp" method="post" onSubmit="return inquiryCheck()">
	<input type="hidden" name="iid" value="<%= sid %>">
	<input type="hidden" name="ref" value="<%= ib.getRef() %>">
	<input type="hidden" name="re_step" value="<%= ib.getRe_step() %>">
	<input type="hidden" name="re_level" value="<%= ib.getRe_level() %>">
	<input type="hidden" name="pageNum" value="<%= pageNum %>">
	<table class="itable" border="0">
		<tr>
			<td>제목</td>
			<td width="400"><input type="text" class="form-control" name="subject" id="i1" value="[답변]<%= ib.getSubject() %>"></td>
		</tr>
		...
		<tr>
			<td colspan="2" class="wtable">
				...
				<input type="button" value="목록 보기" onclick="location.href='<%= request.getContextPath() %>/MY/Inquiry/inquiry.jsp?pageNum=<%= pageNum %>';">
			</td>
		</tr>
